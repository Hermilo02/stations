...
def procesar_datos_api(mac_address):
    ...
    dias_sin_datos = 0  # Contador para días sin datos

    while dias_sin_datos < 3:  # Detener después de 3 días sin datos
        end_date_str = end_date.strftime("%Y-%m-%dT%H:%M:%SZ")
        print(f"Consultando datos para la fecha: {end_date_str}")
        
        df = obtener_datos_api(mac_address, end_date_str)
        
        if df.empty:
            print(f"No se encontraron datos en la API para la fecha: {end_date_str}")
            dias_sin_datos += 1
        else:
            dias_sin_datos = 0  # Reiniciar el contador si hay datos
        ...
        conexion.commit()
        print(f"Datos importados correctamente para la fecha: {end_date_str}")
